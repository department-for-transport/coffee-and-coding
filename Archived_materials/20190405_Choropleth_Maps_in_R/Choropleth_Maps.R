...
#Local authority district shapefiles available from:
...
# so that we don't have to call them each time we produce a map, we can create our own theme
...
# Our colours now change to the default dicrete colour scale but we will look 
# at how to change these a bit later.

# if we want more choice of breaks, we can select our breaks using the classIntervals() 
# function from the classInt package
...
# we can now give the legend a suitable title
ggplot() + 
  geom_polygon(data = map_data, 
               aes(x = long, y = lat, group = group, 
                   fill = cut(x = Percentage, n = 5, breaks = jenks_breaks$brks,
...
dft_map + 
  geom_text(data=LA_labels, aes(long, lat, label = Local_Authority), 
            size=2, fontface="bold")

# With 326 local authorities, plotting them all at once isn't a great idea
# We can choose individual LAs

LA_labels_selected <- LA_labels %>% 
  filter(Local_Authority %in% c("Manchester", "Shropshire", "York", "County Durham"))

dft_map + 
  geom_text(data=LA_labels_selected, aes(long, lat, label = Local_Authority), 
            size=2, fontface="bold")

# not very visible, so we can turn them into labels

dft_map +
  geom_label(data=LA_labels_selected, aes(long, lat, label = Local_Authority),
             size=2, fontface="bold")

# can change the transparency
dft_map +
  geom_label(data=LA_labels_selected, aes(long, lat, label = Local_Authority), 
             alpha= 0.7, size=2, fontface="bold")
